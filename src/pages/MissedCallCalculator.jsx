...
function Field({ label, hint, prefix, suffix, value, onChange, min, max, step }) {
  return (
    <label className="block">
      ...
      <div className="flex items-center rounded-xl border border-white/15 bg-white/[0.04] focus-within:border-[#00A693]">
        {prefix && <span className="pl-4 text-gray-400">{prefix}</span>}
        <input
          type="number"
          inputMode="decimal"
          min={min}
          ...
          className="w-full bg-transparent px-3 py-3 text-white outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
        />
    ...
    <div className="w-full bg-[#0a0a0a] text-gray-100">
      ...
      <section className="px-6 pb-8">
        <div className="max-w-4xl mx-auto grid lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl border border-[#00A693]/30 bg-[#00A693]/[0.07] p-6 sm:p-8 flex flex-col justify-center">
            <p className="text-sm uppercase tracking-wider text-[#00A693] font-semibold">
              Revenue attached to missed calls
            </p>
            <p className="mt-3 text-4xl sm:text-5xl font-bold text-white tabular-nums">
              {money(lostPerYear)}
            </p>
            <p className="text-gray-300">per year</p>

            <dl className="mt-6 space-y-2 border-t border-white/10 pt-5 text-[0.97rem]">
              <div className="flex justify-between gap-4">
                <dt className="text-gray-300">Per month</dt>
                <dd className="text-white font-semibold tabular-nums">{money(lostPerMonth)}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-gray-300">Per week</dt>
                <dd className="text-white font-semibold tabular-nums">{money(lostPerWeek)}</dd>
              </div>
              <div className="flex justify-between gap-4">
                <dt className="text-gray-300">Jobs per year</dt>
      ...
      <section className="px-6 pb-16">
        <div className="max-w-4xl mx-auto rounded-2xl border border-white/10 bg-white/[0.02] p-6 sm:p-8">
          <h2 className="text-xl font-bold text-white mb-4">How that number is worked out</h2>
          <p className="font-mono text-sm sm:text-[0.95rem] text-gray-200 leading-relaxed break-words">
            {calls.toLocaleString("en-US")} missed calls/week × {rate}% who would have
            booked × {money(value)} per job × {WEEKS_PER_YEAR} weeks ={" "}
            <span className="text-[#00A693] font-semibold">{money(lostPerYear)}</span>
          </p>
          <p className="mt-4 text-gray-400 text-[0.95rem] leading-relaxed">
          ...
          <ol className="space-y-5">
            {[
              ...
              <li key={i} className="flex gap-4">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#00A693]/50 text-sm font-semibold text-[#00A693]">
                  {i + 1}
                </span>
                <div>
      ...
      <section className="px-6 py-12">
        <motion.div {...fadeUp} className="max-w-3xl mx-auto">
          ...
          <p className="text-gray-300 leading-relaxed text-[1.05rem] mb-5">
            ...
            <Link to="/services/missed-call-text-back" className="text-[#00A693] underline underline-offset-2">
              Missed call text back
            </Link>{" "}
            sends the caller a text within about a minute, before they dial the next
            company. For higher call volumes, an{" "}
            <Link to="/services/ai-voice-agents" className="text-[#00A693] underline underline-offset-2">
              AI voice agent
            </Link>{" "}
          ...
          <p className="text-gray-300 leading-relaxed text-[1.05rem]">
            ...
            if you are running{" "}
            <Link to="/services/google-ads" className="text-[#00A693] underline underline-offset-2">
              Google Ads
            </Link>{" "}
